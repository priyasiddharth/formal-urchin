...
theorem osea_rhs_direct_run_ok
  ...
  have h_src_ne_res : ctx.srcReg ≠ resReg ctx := by
    intro h_eq
    exact (alloc_fresh_reg (cs := ctx.cs) ctx.src.base ctx.srcBaseLayout)
...
theorem starts_rhs_direct_instrs
  ...
  · have h_start_full :
    ...
    exact ⟨by simpa [Nat.zero_add, List.get?] using (h_start_full 0).symm,
      by simpa [Nat.zero_add, List.get?] using (h_start_full 1).symm,
      by simpa [Nat.zero_add, List.get?] using (h_start_full 2).symm⟩
  · have h_start_full :
        StartsAt prog_osea pc
          [oseair.Instr.Assgn (resReg ctx) (oseair.Rhs.Alloc (innerTy ctx)),
           oseair.Instr.Assgn (loadedPtrReg ctx) (oseair.Rhs.Load TyVal.PTy ctx.srcReg),
...
theorem starts_rhs_projected_instrs
  ...
  · have h_start_full :
    ...
    exact ⟨by simpa [Nat.zero_add, List.get?] using (h_start_full 0).symm,
      by simpa [Nat.zero_add, List.get?] using (h_start_full 1).symm,
      by simpa [Nat.zero_add, List.get?] using (h_start_full 2).symm,
      by simpa [Nat.zero_add, List.get?] using (h_start_full 3).symm,
      by simpa [Nat.zero_add, List.get?] using (h_start_full 4).symm⟩
  · have h_start_full :
        StartsAt prog_osea pc
          [oseair.Instr.Assgn (resReg ctx) (oseair.Rhs.Alloc (innerTy ctx)),
           oseair.Instr.Assgn (srcTmpReg ctx) (oseair.Rhs.BorOffset ctx.srcReg ctx.srcOff),
...
theorem simulation
  ...
  (h_temp_unmapped :
    ∀ a, ρa a ≠ some ((A_o.alloc s_osea.mem (blockSize ctx.innerLayout)).1))
  (h_temp_stack_empty :
    s_osea.ap.StackMap.find? ((A_o.alloc s_osea.mem (blockSize ctx.innerLayout)).1) = none) :
  ∃ s_osea_next,
  ...
  let allocBase_o := (A_o.alloc s_osea.mem (blockSize ctx.innerLayout)).1
  let allocMem_o := (A_o.alloc s_osea.mem (blockSize ctx.innerLayout)).2
  have h_alloc_pair :
      A_o.alloc s_osea.mem (blockSize ctx.innerLayout) = (allocBase_o, allocMem_o) := by
    simp [allocBase_o, allocMem_o]
  have h_alloc_mMap_o : allocMem_o.mMap = s_osea.mem.mMap := by
    have h_alloc_mMap :=
      oseair.AllocatorSpec.alloc_mMap_eq A_o s_osea.mem (blockSize ctx.innerLayout)
    rw [h_alloc_pair] at h_alloc_mMap
    simpa using h_alloc_mMap
  have h_temp_unmapped_alloc : ∀ a, ρa a ≠ some allocBase_o := by
    simpa [allocBase_o] using h_temp_unmapped
  have h_temp_stack_empty_alloc : s_osea.ap.StackMap.find? allocBase_o = none := by
    simpa [allocBase_o] using h_temp_stack_empty
  ...
  have ⟨allocTag_o, ap_alloc, h_osea_alloc⟩ :=
    sb_osea_only_own_ok (Or.inl h_temp_stack_empty_alloc)
  have h_sb_alloc :=
  ...
  ·
    ...
    let s_osea_pre : oseair.State :=
      { s_osea with
        ...
        pc := s_osea.pc + 3 }
    have h_prefix_run :
        oseair.runNWith A_o 3 s_osea prog_osea = oseair.Result.Ok s_osea_pre := by
      simpa [s_osea_pre, vals_temp] using
        (osea_rhs_direct_run_ok (A_o := A_o) ctx
          s_osea prog_osea
          srcBase_o srcTag_o
    ...
    have h_loaded_fresh :
      ...
      simpa [loadedPtrReg, DerefProjectedExistingCtx.loadedPtrReg, h_src_off] using
        (alloc_fresh_reg (cs := { ctx.cs with nextReg := ctx.cs.nextReg + 1 }))
    have h_dstTmp_fresh :
        ∀ base layout,
      ...
      simpa [dstTmpReg, DerefProjectedExistingCtx.dstTmpReg, h_src_off] using
        (alloc_fresh_reg (cs := { ctx.cs with nextReg := ctx.cs.nextReg + 2 }))
    have h_sim_pre_res :
        StateSim ctx.cs.placeMap ρa ρt
          s_mir_pre
          { s_osea with
    ...
    have h_dst_ne_res : ctx.dstReg ≠ resReg ctx := by
      intro h_eq
      exact (alloc_fresh_reg (cs := ctx.cs) ctx.dst.base ctx.dstBaseLayout)
    ...
    have h_dst_ne_loaded : ctx.dstReg ≠ loadedPtrReg ctx := by
      intro h_eq
      exact (alloc_fresh_reg (cs := { ctx.cs with nextReg := ctx.cs.nextReg + 1 })
    ...
    obtain ⟨s_osea_next, h_steps, h_sim_next⟩ :=
  ...
  ·
    ...
    let s_osea_pre : oseair.State :=
      { s_osea with
        ...
        pc := s_osea.pc + 5 }
    have h_prefix_run :
        oseair.runNWith A_o 5 s_osea prog_osea = oseair.Result.Ok s_osea_pre := by
      simpa [s_osea_pre, vals_temp] using
        (osea_rhs_projected_run_ok (A_o := A_o) ctx
          s_osea prog_osea
    ...
    have h_dstTmp_fresh :
        ∀ base layout,
          ctx.cs.placeMap.lookup base = some (dstTmpReg ctx, layout) → False := by
      simpa [dstTmpReg, DerefProjectedExistingCtx.dstTmpReg, h_src_off] using
        (alloc_fresh_reg (cs := { ctx.cs with nextReg := ctx.cs.nextReg + 3 }))
    have h_sim_pre_res :
        StateSim ctx.cs.placeMap ρa ρt
          s_mir_pre
          { s_osea with
    ...
    have h_dst_ne_loaded : ctx.dstReg ≠ loadedPtrReg ctx := by
      intro h_eq
      exact (alloc_fresh_reg (cs := { ctx.cs with nextReg := ctx.cs.nextReg + 2 })
        ctx.dst.base ctx.dstBaseLayout)
        (by simpa [loadedPtrReg, DerefProjectedExistingCtx.loadedPtrReg, h_src_off, h_eq]
          using ctx.h_dst_lookup)
    have h_dst_reg_pre :
...
theorem starts_direct_instrs
  ...
  prog_osea.get? pc =
    ...
    prog_osea.get? (pc + 2) =
      some (oseair.Instr.Memcpy (resReg ctx) (loadedPtrReg ctx) (innerTy ctx)) := by
  refine ⟨?_, ?_, ?_⟩
  · simpa [DerefFreshCtx.compiled, h_src_off, Nat.zero_add, List.get?] using (h_start 0).symm
  · simpa [DerefFreshCtx.compiled, h_src_off, List.get?] using (h_start 1).symm
  · simpa [DerefFreshCtx.compiled, h_src_off, List.get?] using (h_start 2).symm
...
theorem starts_projected_instrs
  ...
  prog_osea.get? pc =
    ...
    prog_osea.get? (pc + 4) =
      some (oseair.Instr.Memcpy (resReg ctx) (loadedPtrReg ctx) (innerTy ctx)) := by
  refine ⟨?_, ?_, ?_, ?_, ?_⟩
  · simpa [DerefFreshCtx.compiled, if_neg h_src_off, Nat.zero_add, List.get?] using (h_start 0).symm
  · simpa [DerefFreshCtx.compiled, if_neg h_src_off, List.get?] using (h_start 1).symm
  · simpa [DerefFreshCtx.compiled, if_neg h_src_off, List.get?] using (h_start 2).symm
  · simpa [DerefFreshCtx.compiled, if_neg h_src_off, List.get?] using (h_start 3).symm
  · simpa [DerefFreshCtx.compiled, if_neg h_src_off, List.get?] using (h_start 4).symm
...
theorem starts_direct_suffix
  ...
  intro i
  simpa [DerefFreshCtx.compiled, h_src_off, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
    using h_start (i + 3)
...
theorem starts_projected_suffix
  ...
  StartsAt prog_osea (pc + 5)
    [oseair.Instr.Assgn (dstReg ctx) (oseair.Rhs.Alloc (innerTy ctx)),
     oseair.Instr.Memcpy (dstReg ctx) (resReg ctx) (innerTy ctx)] := by
  intro i
  simpa [DerefFreshCtx.compiled, if_neg h_src_off, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
    using h_start (i + 5)
...
theorem finish_from_pre
  ...
  ∃ ρa' ρt' s_osea_next,
    ...
    StateSim (postPlaceMap ctx) ρa' ρt' s_mir_next s_osea_next ptr_sim := by
  ...
  simpa [postPlaceMap, DerefFreshCtx.postPlaceMap, s_osea_post, ρa', ρt'] using
    state_sim_alloc_write
      ...
      h_sim_pre rfl rfl h_dstReg_fresh h_alloc_mMap_m h_alloc_mMap_o h_sb_write h_vals h_len
...
theorem simulation
  ...
  · have ⟨h_stmt0, h_stmt1, h_stmt2⟩ :=
    ...
    have h_res_fresh :
        ∀ base layout,
          ctx.cs.placeMap.lookup base = some (resReg ctx, layout) → False := by
      simpa [resReg, DerefFreshCtx.resReg] using (alloc_fresh_reg (cs := ctx.cs))
    have h_loaded_fresh :
        ∀ base layout,
          ctx.cs.placeMap.lookup base = some (loadedPtrReg ctx, layout) → False := by
    ...
    obtain ⟨ρa', ρt', s_osea_next, h_steps, h_sim_next⟩ :=
      finish_from_pre
        (A_o := A_o) ctx
        (ptr_sim := ptr_sim)
        (s_mir_pre := s_mir_pre)
        ...
        h_sim_pre h_temp_unmapped_alloc h_find_temp_pre
        h_start_suffix h_src_reg_pre h_dstReg_fresh
        h_mir_own h_mir_use h_alloc_mMap_m h_mir_next_pre h_vals h_len
    exact ⟨ρa', ρt', s_osea_next,
      StepStarWith.trans (StepStarWith.of_runN_ok h_prefix_run) h_steps,
      h_sim_next⟩
  · have ⟨h_stmt0, h_stmt1, h_stmt2, h_stmt3, h_stmt4⟩ :=
...
theorem simulation
  ...
  obtain ⟨s_osea_next, h_steps, h_sim_next⟩ :=
    DerefProjectedExisting.simulation
      ...
      h_sim' h_mir_start' h_mir_step h_pointee_tracked'
      h_osea_start' h_inner_nonempty h_temp_unmapped h_temp_stack_empty
