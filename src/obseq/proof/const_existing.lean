...
structure ConstExistingCtx where
  ...
  /-- The compiler state. -/
  cs : CompilerState
  /-- Every mapped register is strictly below `nextReg`. -/
...
theorem simulation
  ...
  have h_tmp_fresh :
    ...
    by
      simpa [ConstExistingCtx.tmpReg] using
        (alloc_fresh_reg_of_ge (cs := ctx.cs) ctx.h_regs_below (Nat.le_refl _))
  obtain ⟨s_osea_next, h_steps, h_rest⟩ := existing_write_simulation h_sim h_noninterference h_ptr h_fit (by decide) h_tmp_fresh
