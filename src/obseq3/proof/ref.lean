...
/-- RESIDUAL (sorried), NARROWED 2026-08-28: what remains after P→L and
    D→L (deref source through a load spine) closed.
    - DEREF SOURCE, non-spine pointer place or UNBOUND dst: `*p` where
      `p` involves projections (needs the interleaved `Borrow;…;Die`
      cleanup commutation) or the regime-B fresh-dst composition.
