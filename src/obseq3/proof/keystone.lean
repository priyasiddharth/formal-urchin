...
The copy fragment `[Borrow(Shared); Memcpy; Die]` executes the dst
`useMut` (inside the atomic `Memcpy`) BETWEEN the keystone's read and
die phases. On disjoint ranges each operation touches only its own
cells, so the die commutes past the write — stated at the `find?`
level, because `SB.set`'s move-to-front reorders the LIST differently
under the two orders (`StackMapSim` went find?-based for exactly this
reason). Disjointness is supplied by the overlapping-assignment
guard. -/
...
theorem chain_key_not_mem {W : Nat → BorrowStack} {addr b : Word}
  ...
  · rw [chain_step hi]
    ...
    · exact h i (Nat.le_refl i) hi h1
    · exact chain_key_not_mem (fun j hj1 hj2 => h j (by omega) hj2) h2
  · rw [chain_stop hi]
    simp [keysOf]
  termination_by len - i

/-- `sb_write` congruence in the observation fields: two states with the
    same StackMap, protFrames and exposed produce the same written map;
    the NextTag is each state's own. -/
theorem sb_write_congr {p q p' : AccessPerms} {addr : Word} {len : Nat}
    {tag : Tag}
    (h_sm : q.StackMap = p.StackMap)
    (h_pf : q.protFrames = p.protFrames)
    (h_ex : q.exposed = p.exposed)
    (h : sb_write p addr len tag = .ok p') :
    ∃ q', sb_write q addr len tag = .ok q' ∧
      q'.StackMap = p'.StackMap ∧
      q'.protFrames = q.protFrames ∧ q'.exposed = q.exposed ∧
      q'.NextTag = q.NextTag := by
  have h0 : foldCells (fun ap a => writeCell ap a tag) p (addr + 0) len
      = .ok p' := h
  obtain ⟨V, Wc, h_cells, h_p'⟩ :=
    foldCells_ok_inv
      (C := fun a stack => writeCellContent p.protFrames p.exposed a tag stack)
      (msgNone := fun a => s!"sb-write: no borrow stack at address {a}")
      (P := p.protFrames) (E := p.exposed) (N := p.NextTag)
      (fun ap a h_pf' h_ex' _ => writeCell_content_form tag ap a h_pf' h_ex')
      len 0 p p' rfl rfl rfl h0
  simp only [Nat.zero_add] at h_cells h_p'
  have h_tgt := foldCells_ok_of_cells
      (op := fun ap a => writeCell ap a tag)
      (C := fun a stack => writeCellContent p.protFrames p.exposed a tag stack)
      (msgNone := fun a => s!"sb-write: no borrow stack at address {a}")
      (P := q.protFrames) (E := q.exposed) (N := q.NextTag)
      (fun ap a h_pf' h_ex' _ => by
        show writeCell ap a tag = _
        rw [writeCell_content_form tag ap a h_pf' h_ex']
        simp only [h_pf, h_ex])
      len 0 q V Wc rfl rfl rfl
      (fun j h1 h2 => by
        rw [h_sm]
        exact (h_cells j h1 (by omega)).1)
      (fun j h1 h2 => (h_cells j h1 (by omega)).2)
  rw [show (0 : Nat) + len = len from Nat.zero_add len] at h_tgt
  refine ⟨_, h_tgt, ?_, rfl, rfl, rfl⟩
  rw [h_p', h_sm]

/-- Disjoint-range commutation, die past write: from the KNOWN order
    (die the src range, then write the dst range) recover the EXECUTED
    order (write, then die) with a `find?`-identical result. -/
theorem sb_die_sb_write_comm
    {p q r : AccessPerms} {aD aW : Word} {nD nW : Nat} {tD tW : Tag}
    (h_dis : ∀ j k, j < nD → k < nW → aD + j ≠ aW + k)
    (h_die : sb_die p aD nD tD = .ok q)
    (h_write : sb_write q aW nW tW = .ok r) :
    ∃ w r', sb_write p aW nW tW = .ok w ∧ sb_die w aD nD tD = .ok r' ∧
      (∀ a, SB.find? r'.StackMap a = SB.find? r.StackMap a) ∧
      r'.protFrames = r.protFrames ∧ r'.exposed = r.exposed ∧
      r'.NextTag = r.NextTag := by
  -- §1 characterize the die on p
  have h_die0 : foldCells
      (fun ap a =>
        match SB.find? ap.StackMap a with
        | none => Except.error s!"sb-die: no borrow stack at address {a}"
        | some stack =>
            match dieCellContent ap.protFrames tD stack with
            | .error e => .error e
            | .ok below => .ok { ap with StackMap := SB.set ap.StackMap a below })
      p (aD + 0) nD = .ok q := h_die
  obtain ⟨VD, WD, h_cellsD, h_q⟩ :=
    foldCells_ok_inv
      (C := fun _ stack => dieCellContent p.protFrames tD stack)
      (msgNone := fun a => s!"sb-die: no borrow stack at address {a}")
      (P := p.protFrames) (E := p.exposed) (N := p.NextTag)
      (fun ap a h_pf' h_ex' _ => by
        cases h_find : SB.find? ap.StackMap a with
        | none => simp only [h_find]
        | some stack =>
            cases h_content : dieCellContent p.protFrames tD stack with
            | error e => simp only [h_pf', h_find, h_content]
            | ok below => simp only [h_pf', h_find, h_content])
      nD 0 p q rfl rfl rfl h_die0
  simp only [Nat.zero_add] at h_cellsD h_q
  have h_q_pf : q.protFrames = p.protFrames := by rw [h_q]
  have h_q_ex : q.exposed = p.exposed := by rw [h_q]
  have h_q_nt : q.NextTag = p.NextTag := by rw [h_q]
  have h_q_sm : q.StackMap = setChain p.StackMap (chain WD aD 0 nD) := by rw [h_q]
  -- §2 characterize the write on q; its inputs are p's cells (disjoint)
  have h_w0 : foldCells (fun ap a => writeCell ap a tW) q (aW + 0) nW
      = .ok r := h_write
  obtain ⟨VW, WW, h_cellsW, h_r⟩ :=
    foldCells_ok_inv
      (C := fun a stack => writeCellContent p.protFrames p.exposed a tW stack)
      (msgNone := fun a => s!"sb-write: no borrow stack at address {a}")
      (P := q.protFrames) (E := q.exposed) (N := q.NextTag)
      (fun ap a h_pf' h_ex' _ => by
        show writeCell ap a tW = _
        rw [writeCell_content_form tW ap a h_pf' h_ex']
        simp only [h_q_pf, h_q_ex])
      nW 0 q r rfl rfl rfl h_w0
  simp only [Nat.zero_add] at h_cellsW h_r
  have h_r_pf : r.protFrames = p.protFrames := by rw [h_r, h_q_pf]
  have h_r_ex : r.exposed = p.exposed := by rw [h_r, h_q_ex]
  have h_r_nt : r.NextTag = p.NextTag := by rw [h_r, h_q_nt]
  have h_findW : ∀ k, k < nW →
      SB.find? p.StackMap (aW + k) = some (VW k) := by
    intro k hk
    have h1 := (h_cellsW k (Nat.zero_le k) (by omega)).1
    rw [h_q_sm, setChain_find?_not_mem _ _
      (chain_key_not_mem (fun j hj1 hj2 h_eq =>
        h_dis j k hj2 hk h_eq.symm))] at h1
    exact h1
  -- §3 the write on p
  have h_wp := foldCells_ok_of_cells
      (op := fun ap a => writeCell ap a tW)
      (C := fun a stack => writeCellContent p.protFrames p.exposed a tW stack)
      (msgNone := fun a => s!"sb-write: no borrow stack at address {a}")
      (P := p.protFrames) (E := p.exposed) (N := p.NextTag)
      (fun ap a h_pf' h_ex' _ => writeCell_content_form tW ap a h_pf' h_ex')
      nW 0 p VW WW rfl rfl rfl
      (fun k h1 h2 => h_findW k (by omega))
      (fun k h1 h2 => (h_cellsW k h1 (by omega)).2)
  -- §4 the die on the written state
  have h_dw := foldCells_ok_of_cells
      (op := fun ap a =>
        match SB.find? ap.StackMap a with
        | none => Except.error s!"sb-die: no borrow stack at address {a}"
        | some stack =>
      ...
      (fun ap a h_pf' h_ex' _ => by
        ...
        | some stack =>
            ...
            | ok below => simp only [h_pf', h_find, h_content])
      nD 0
      { p with StackMap := setChain p.StackMap (chain WW aW 0 nW) }
      VD WD rfl rfl rfl
      (fun j h1 h2 => by
        show SB.find? (setChain p.StackMap (chain WW aW 0 nW)) (aD + j)
          = some (VD j)
        rw [setChain_find?_not_mem _ _
          (chain_key_not_mem (fun k hk1 hk2 h_eq =>
            h_dis j k (by omega) hk2 h_eq))]
        exact (h_cellsD j h1 (by omega)).1)
      (fun j h1 h2 => (h_cellsD j h1 (by omega)).2)
  rw [show (0 : Nat) + nW = nW from Nat.zero_add nW] at h_wp
  rw [show (0 : Nat) + nD = nD from Nat.zero_add nD] at h_dw
  refine ⟨_, _, h_wp, h_dw, ?_, ?_, ?_, ?_⟩
  · -- find?-equality of the two application orders
    intro a
    rw [h_r, h_q_sm]
    show SB.find? (setChain (setChain p.StackMap (chain WW aW 0 nW))
        (chain WD aD 0 nD)) a
      = SB.find? (setChain (setChain p.StackMap (chain WD aD 0 nD))
        (chain WW aW 0 nW)) a
    by_cases hD : aD ≤ a ∧ a < aD + nD
    · have hj : a = aD + (a - aD) := by grind
      rw [hj]
      rw [setChain_chain_find? _ (a - aD) (Nat.zero_le _) (by grind),
        setChain_find?_not_mem _ _
          (chain_key_not_mem (fun k hk1 hk2 h_eq =>
            h_dis (a - aD) k (by grind) hk2 h_eq)),
        setChain_chain_find? _ (a - aD) (Nat.zero_le _) (by grind)]
    · by_cases hW : aW ≤ a ∧ a < aW + nW
      · have hk : a = aW + (a - aW) := by grind
        rw [hk]
        rw [setChain_find?_not_mem _ _
            (chain_key_not_mem (fun j hj1 hj2 h_eq => by
              exact h_dis j (a - aW) hj2 (by grind) h_eq.symm)),
          setChain_chain_find? _ (a - aW) (Nat.zero_le _) (by grind),
          setChain_chain_find? _ (a - aW) (Nat.zero_le _) (by grind)]
      · rw [setChain_find?_not_mem _ _
            (chain_key_not_mem (fun j hj1 hj2 h_eq => by grind)),
          setChain_find?_not_mem _ _
            (chain_key_not_mem (fun k hk1 hk2 h_eq => by grind)),
          setChain_find?_not_mem _ _
            (chain_key_not_mem (fun k hk1 hk2 h_eq => by grind)),
          setChain_find?_not_mem _ _
            (chain_key_not_mem (fun j hj1 hj2 h_eq => by grind))]
