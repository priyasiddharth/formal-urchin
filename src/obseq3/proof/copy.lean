...
/-- RESIDUAL (sorried), NARROWED 2026-09-03 (temp-assignment
    ...
    Remaining: the NON-LOCAL destination (`(*p).f := copy s`). The
    event-ORDER obstacle that used to block it is GONE — the copy now
    reads into a register in the rhs pre-phase, before the destination
    is lowered, so both machines are read-then-write and the two orders
    coincide (that also retired the overlap guard, and d59 pins the
    divergence it used to cause). What is left is genuine composition
    work: the leaf must run TWO place lowerings (source, then
    destination) and both cleanups. -/
