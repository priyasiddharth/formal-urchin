...
theorem compileStmt_copy_chaindst_projsrc_zero_run
    ...
    (h_dval : CheckedCompilerM.value (placeToRegChecked RefKind.Mut (Place.deref P))
      (emit
        { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg + 1,
          nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextLabel,
          code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).code,
          placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).placeRegMap }
        [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg)
...
theorem compileStmt_copy_chaindst_projsrc_offset_run
  ...
  simp only [compileStmtChecked, compileRExprPreChecked,
    ...
    CheckedCompilerM.run_lift, CheckedCompilerM.value_lift,
    CheckedCompilerM.run_pure, CheckedCompilerM.value_pure,
    h_root, h_bval, dif_neg h_o]
  simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
    cleanupInstrs, h_bclean, List.nil_append, List.reverse_cons,
  ...
  · rw [if_neg h_fit] at h_step
    ...
    obtain ⟨dOut0, h_dval0⟩ := placeToRegChecked_ok_of_placeInputsMapped
      (cs := (emit
        { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1,
          nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel,
          code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code,
          placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap }
        ([Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
            (Rhs.Load (layoutToTyVal τ) sOut0.result.reg)]
          ++ cleanupInstrs sOut0.result.cleanup))) (kind := RefKind.Mut)
      (PlaceInputsMapped.placeRegMap_congr (by simp only [emit]; exact h_prmS)
    ...
    have h_incrS : StateIncr (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix)
      ...
      simp only [compileStmtChecked, compileRExprPreChecked,
        CheckedCompilerM.run_bind, CheckedCompilerM.value_bind,
        CheckedCompilerM.run_lift, CheckedCompilerM.value_lift,
        CheckedCompilerM.run_pure, CheckedCompilerM.value_pure, h_root, h_pv0, h_pr0]
      simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM]
      ...
      · exact StateIncr.trans
          ...
          (CheckedCompilerM.incr (placeToRegChecked RefKind.Mut (Place.deref P)) _)
    have h_instS : ∀ q' instr,
        q' < (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel →
        (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code q' = some instr →
        compProg q' = some instr := by
      intro q' instr h_lt h_code
      refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
      · exact Nat.lt_of_lt_of_le h_lt h_incrS.nextLabel_le
    ...
    obtain ⟨p2, h_read_tgt, h_psim2⟩ :=
      sb_read_respects_PermSim h_spsim h_wf_t h_srt h_snw h_read_src
    have h_code1 : compProg s_mid1.pc
        = some (Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (Rhs.Load (layoutToTyVal τ) sOut.result.reg)) := by
      rw [h_spc]
      refine h_instD _ _ ?_ ?_
      · simp only [emit, List.length_cons, List.length_nil]
        omega
      · have h := emit_code_at_new
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1,
            ...
            code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code,
            placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap }
          [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (Rhs.Load (layoutToTyVal τ) sOut.result.reg)]
          (k := 0) (by simp)
        simpa using h
    have h_read2t : MSB.read s_mid1.perms
        (rs.allocBase + (rs.addr - rs.allocBase))
      ...
      exact h_read_tgt
    have h_run1 := runN_Assgn_Load_ptr_step compProg s_mid1
      (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) sOut.result.reg (layoutToTyVal τ) h_code1 h_sentry
      (by rw [h_ts]; grind) h_read2t
    rw [h_ts, h_cancelS] at h_run1
    -- §7 the DESTINATION mother lemma, at the post-read states
    have h_prmCS1 : (emit
    ...
    have h_lbs1 : LocalBindingSim ρa ρt s_mir.env
        { s_mid1 with
          perms := p2,
          reg := oseair.RegMap.insert s_mid1.reg (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
            (layoutToTyVal τ, (oseair.readWordSeq s_mid1.mem rs.addr (blockSize τ))),
          pc := s_mid1.pc + 1 } (emit
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1,
            nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel,
      ...
      · rename_i perms₃ h_useMut_src
        ...
        have h_regbelow : RegisterBelow
            (emit
              ...
              [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
                (Rhs.Load (layoutToTyVal τ) sOut.result.reg)]).nextReg
            (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) := by
          show (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg < _
          simp only [emit]
          omega
        have h_vreg : oseair.RegMap.lookup s_mid2.reg (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
            = some (layoutToTyVal τ, (oseair.readWordSeq s_mid1.mem rs.addr (blockSize τ))) := by
          rw [h_dframe (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) h_regbelow]
          exact RegMap.lookup_insert_self _ _ _
        have h_code2 : compProg s_mid2.pc
            = some (Instr.RStore (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) dOut.result.reg) := by
          rw [h_dpc]
          refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
          · rw [h_stmtRun]
            show _ < _ + 1
          ...
          (emit
            ...
            [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
              (Rhs.Load (layoutToTyVal τ) sOut.result.reg)]))
              [Instr.RStore (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) dOut.result.reg]
              (k := 0) (by simp)
            simpa using h
        have h_useMut2t : MSB.useMut s_mid2.perms
            (rd.allocBase + (rd.addr - rd.allocBase)) (oseair.readWordSeq s_mid1.mem rs.addr (blockSize τ)).length tresD = .ok p3 := by
        ...
        have h_wtp : oseair.writeThroughPtr MSB s_mid2 dOut.result.reg (oseair.readWordSeq s_mid1.mem rs.addr (blockSize τ))
          ...
          simp only [h_useMut2t, h_cancelD]
        have h_run2 := runN_RStore_step compProg s_mid2 _
          (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) dOut.result.reg (oseair.readWordSeq s_mid1.mem rs.addr (blockSize τ))
          _ h_code2 h_vreg h_dentry h_wtp
        have h_runA := (oseair_runN_add n1 1 s_osea compProg s_mid1 h_srun).trans h_run1
        have h_runB := (oseair_runN_add (n1 + 1) n2 s_osea compProg _ h_runA).trans h_drun
        have h_run := (oseair_runN_add (n1 + 1 + n2) 1 s_osea compProg s_mid2 h_runB).trans
...
theorem copy_chaindst_projsrc_offset_simulation
  ...
  · rw [if_neg h_fit] at h_step
    ...
    have h_incrCS2V : StateIncr 
      ...
      · rename_i a h_a
        exact StateIncr.trans
          (CheckedCompilerM.incr
            (placeToRegChecked RefKind.Mut (Place.deref P)) _)
          (emit_tower_incr₃ _ _ _ _)
      · exact CheckedCompilerM.incr
          (placeToRegChecked RefKind.Mut (Place.deref P)) _
    have h_incrS : StateIncr (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix)
    ...
    have h_incrCS2 : StateIncr 
      ...
      simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
        cleanupInstrs, h_sclean, List.nil_append, List.cons_append,
                ...
                (emit
                  { (emit
                    ...
                    [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
                      ...
                      nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 + 1 }
                  [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)),
                   Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).nextLabel →
                ...
                (emit
                  { (emit
                    ...
                    [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
                      ...
                      nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 + 1 }
                  [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)),
                   Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).code q' = some instr →
        compProg q' = some instr := by
      intro q' instr h_lt h_code
      refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
                ...
                (emit
                  { (emit
                    ...
                    [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
                      ...
                      nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 + 1 }
                  [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)),
                   Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).placeRegMap = csPrefix.placeRegMap := by
      simp only [emit]
      exact h_sprm
                ...
                (emit
                  { (emit
                    ...
                    [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
                      ...
                      nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 + 1 }
                  [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)),
                   Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]) := by
      intro τ' loc' binding' h_env'
      obtain ⟨reg', base', tag', h_pi', h_entry', h_ra', h_rt', h_nw', h_dom'⟩ :=
    ...
    obtain ⟨dOut, n2, s_mid2, tresD, h_dval, h_dclean, h_drun, h_dpc, h_dmem,
      ...
      h_dbelow, h_dprm, h_dregmono, h_dlabmono, h_dframe, -⟩ :=
      ptrChain_lowering_sim (s_mir := { s_mir with perms := perms₂ })
        (compProg := compProg) h_id_a h_wf_t h_dchain RefKind.Mut 
                (emit
                  { (emit
                    { (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix) with
                        nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 }
        ...
        (by
          ...
          simp only [emit, List.length_cons, List.length_nil])
        h_instDst
    -- §9 the WRITE: transport, then execute the `RStore`
    have h_stmtRun := (h_run0 csPrefix).trans
    ...
    · rename_i h_nb
      ...
      · rename_i perms₃ h_useMut_src
                ...
                (emit
                  { (emit
                    ...
                    [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)
                      ...
                      nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 + 1 }
                  [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)),
                   Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).nextReg (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) := by
        ...
        have h_vreg : oseair.RegMap.lookup s_mid2.reg (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1))
          ...
          exact RegMap.lookup_insert_self _ _ _
        have h_code4 : compProg s_mid2.pc
            = some (Instr.RStore (layoutToTyVal τ) (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) dOut.result.reg) := by
          rw [h_dpc]
          refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
          · rw [h_stmtRun]
            show _ < _ + 1
          ...
          · rw [h_stmtRun]
            have h := emit_code_at_new 
                (CheckedCompilerM.run (placeToRegChecked RefKind.Mut (Place.deref P))
                    ...
                    [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)),
                     Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]))
              [Instr.RStore (layoutToTyVal τ) (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) dOut.result.reg]
              (k := 0) (by simp)
            simpa using h
        have h_useMut2t : MSB.useMut s_mid2.perms
            (rd.allocBase + (rd.addr - rd.allocBase)) (oseair.readWordSeq s_mid1.mem (rs.addr + pathOffset spath) (blockSize τ)).length tresD
            = .ok p3 := by
          rw [h_cancelD, oseair_readWordSeq_length]
          simpa only [mirlite_readWordSeq_length] using h_useMut_tgt
        have h_wtp : oseair.writeThroughPtr MSB s_mid2 dOut.result.reg (oseair.readWordSeq s_mid1.mem (rs.addr + pathOffset spath) (blockSize τ))
            "RStore Invalid Regs"
            = oseair.Result.Ok
              { s_mid2 with
                  perms := p3,
                  mem := oseair.writeWordSeq s_mid2.mem rd.addr (oseair.readWordSeq s_mid1.mem (rs.addr + pathOffset spath) (blockSize τ)),
                  pc := s_mid2.pc + 1 } := by
          have h_dl : oseair.RegMap.lookup s_mid2.reg dOut.result.reg
              = some (obseq.TyVal.PTy,
                  [Val.Ptr rd.allocBase (rd.addr - rd.allocBase) rd.allocSize
          ...
          rw [h_cancelD] at h_useMut2t
          simp only [h_useMut2t, h_cancelD]
        have h_run4 := runN_RStore_step compProg s_mid2 _
          (layoutToTyVal τ) (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) dOut.result.reg (oseair.readWordSeq s_mid1.mem (rs.addr + pathOffset spath) (blockSize τ))
          _ h_code4 h_vreg h_dentry h_wtp
        have h_runA := (oseair_runN_add n1 1 s_osea compProg s_mid1 h_srun).trans h_run1
        have h_runB := (oseair_runN_add (n1 + 1) 1 s_osea compProg _ h_runA).trans h_run2
        have h_runC := (oseair_runN_add (n1 + 1 + 1) 1 s_osea compProg _ h_runB).trans h_run3
        have h_runD := (oseair_runN_add (n1 + 1 + 1 + 1) n2 s_osea compProg _ h_runC).trans h_drun
        have h_run := (oseair_runN_add (n1 + 1 + 1 + 1 + n2) 1 s_osea compProg
          s_mid2 h_runD).trans h_run4
        -- §10 memory: the same values land at the same addresses
        have h_memchain : s_mid2.mem = s_osea.mem := by
          rw [h_dmem]
          show s_mid1.mem = _
          exact h_smem
        ...
        have h_sms' : SourceMemSim ρa ρt
            (mirlite.writeWordSeq s_mir.mem rd.addr
              (mirlite.readWordSeq s_mir.mem (rs.addr + pathOffset spath) (blockSize τ)))
            (oseair.writeWordSeq s_mid2.mem rd.addr (oseair.readWordSeq s_mid1.mem (rs.addr + pathOffset spath) (blockSize τ))) := by
          refine SourceMemSim.writeWordSeq_extend h_id_a _ _ _ _ _ h_rel h_dom ?_
          rw [h_memchain]
          exact h_sms
        -- §11 rebuild the invariant
        refine ⟨_, n1 + 1 + 1 + 1 + n2 + 1, h_run, ?_⟩
        refine ⟨CheckedCompilerM.run (compileStmtChecked stmt0) csPrefix,
          ⟨prefixCompileState_succ h_csAt h_stmt h_stmtOut, ?_⟩, ?_, ?_, h_psim3,
          h_id_a, h_wf_t, ?_, ?_, ?_, ?_⟩
        · show s_mid2.pc + 1 = _
...
theorem compileStmt_copy_projlocal_fresh_zero_run
    ...
    (h_dpi : getPlaceInfo (emit
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { cs with nextReg := cs.nextReg + 1 } [Instr.Assgn (Register.R cs.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R cs.nextReg, σ))).nextReg + 1,
            nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { cs with nextReg := cs.nextReg + 1 } [Instr.Assgn (Register.R cs.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R cs.nextReg, σ))).nextLabel,
            code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { cs with nextReg := cs.nextReg + 1 } [Instr.Assgn (Register.R cs.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R cs.nextReg, σ))).code,
            placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { cs with nextReg := cs.nextReg + 1 } [Instr.Assgn (Register.R cs.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R cs.nextReg, σ))).placeRegMap }
  ...
  simp only [compileStmtChecked, h_proj_eq, compileRExprPreChecked,
    ...
    h_root, h_sval]
  simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
    cleanupInstrs, h_sclean, List.reverse_nil, List.map_nil, List.append_nil,
    emit_nil]
  rw [h_brun, h_bval]
...
/-- REGIME B-proj for copy: a PROJECTED destination whose LOCAL root is
    UNBOUND, at ZERO offset (`t.0 := copy y` with `t` fresh).
    `ensurePlaceRoot` allocates the whole σ-sized root before the rhs
    pre-phase runs, so the shape is the root `Alloc`, then the source
    mother lemma at the POST-allocation states under both extended
    renames, then the `Load`, then the `RStore` through the fresh root
  ...
  · rw [if_neg h_fitS] at h_step
    ...
    have h_dpi0 : getPlaceInfo (emit
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1,
            ...
            code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).code,
            placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).placeRegMap }
    ...
    have h_instS : ∀ q' instr,
        q' < (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextLabel →
        (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).code q' = some instr →
        compProg q' = some instr := by
      intro q' instr h_lt h_code
      refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
      · exact Nat.lt_of_lt_of_le h_lt h_incrS.nextLabel_le
    ...
    · rename_i h_nb
      ...
      · rename_i perms₃ h_useMut_src
        ...
        have h_code2 : compProg (s_mid.pc + 1)
          ...
          [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
            (Rhs.Load (layoutToTyVal τ) sOut.result.reg)])
              [Instr.RStore (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
  ...
  · rw [if_neg h_fitS] at h_step
    ...
    have h_incrS1V : StateIncr (emit
      ...
      simp only [h_bval0, h_brun0, h_o, dif_pos,
        ...
        CheckedCompilerM.run_pure, CheckedCompilerM.value_pure]
      simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
        cleanupInstrs, h_bres0, List.reverse_nil, List.map_nil, List.nil_append,
    ...
    · rename_i h_nb
      ...
      · rename_i perms₃ h_useMut_src
        ...
        have h_code2 : compProg (s_mid.pc + 1)
          ...
          · rw [h_stmtRun]
            ...
            rw [emit_code_lt_nextLabel _ _ (by
              simp only [emit, List.length_cons, List.length_nil]; omega)]
            have h := emit_code_at_new
              { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1 + 1,
                nextLabel := (emit
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1,
            nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextLabel,
            code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).code,
          ...
          [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
            (Rhs.Load (layoutToTyVal τ) sOut.result.reg)]).placeRegMap }
              [Instr.Assgn (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1))
                (borrowRhs RefKind.Mut (blockSize τ) (Register.R csPrefix.nextReg)
                  (pathOffset path))]
              (k := 0) (by simp)
            simpa [emit] using h
        have h_code3 : compProg (s_mid.pc + 1 + 1)
            = some (Instr.RStore (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
                (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1))) := by
          rw [h_spc]
          refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
          · rw [h_stmtRun]
            simp only [emit, List.length_cons, List.length_nil]
          ...
          [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
            ...
            simpa [emit] using h
        have h_code4 : compProg (s_mid.pc + 1 + 1 + 1)
            = some (Instr.Die (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1)) (blockSize τ)) := by
          rw [h_spc]
          refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
          · rw [h_stmtRun]
            simp only [emit, List.length_cons, List.length_nil]
          ...
          [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
            (Rhs.Load (layoutToTyVal τ) sOut.result.reg)]).placeRegMap }
                ...
                [Instr.RStore (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg)
                  (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1))])
              [Instr.Die (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (setPlaceInfo (emit { csPrefix with nextReg := csPrefix.nextReg + 1 } [Instr.Assgn (Register.R csPrefix.nextReg) (Rhs.Alloc (layoutToTyVal σ))]) loc.idx.1 (Register.R csPrefix.nextReg, σ))).nextReg + 1)) (blockSize τ)]
              (k := 0) (by simp)
            simpa [emit] using h
        ...
        have h_runB := (oseair_runN_add (1 + n1) 1 s_osea compProg s_mid h_runA).trans
          h_run1
        have h_runC := (oseair_runN_add (1 + n1 + 1) 1 s_osea compProg _ h_runB).trans
          h_run2
        have h_runD := (oseair_runN_add (1 + n1 + 1 + 1) 1 s_osea compProg _ h_runC).trans
          h_run3
        have h_run := (oseair_runN_add (1 + n1 + 1 + 1 + 1) 1 s_osea compProg _
          h_runD).trans h_run4
      ...
      obtain ⟨h_dagr, h_dagv⟩ := placeToRegChecked_flatten_agree
        (Place.proj (Place.deref pp) path) RefKind.Mut (emit
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (ensurePlaceRoot (Place.proj (Place.deref pp) path) cs).snd.val).nextReg + 1,
            nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (ensurePlaceRoot (Place.proj (Place.deref pp) path) cs).snd.val).nextLabel,
            code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (ensurePlaceRoot (Place.proj (Place.deref pp) path) cs).snd.val).code,
            placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) (ensurePlaceRoot (Place.proj (Place.deref pp) path) cs).snd.val).placeRegMap }
...
theorem compileStmt_copy_projdst_zero_run
  ...
  simp only [compileStmtChecked, compileRExprPreChecked, h_proj_eq,
    CheckedCompilerM.run_bind, CheckedCompilerM.value_bind,
    CheckedCompilerM.run_lift, CheckedCompilerM.value_lift,
    CheckedCompilerM.run_pure, CheckedCompilerM.value_pure,
    h_root, h_sval]
...
theorem compileStmt_copy_projdst_offset_value
  ...
  simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM]
  rw [h_dval]
  simp only [dif_neg h_off, CheckedCompilerM.run_bind,
    CheckedCompilerM.value_bind, CheckedCompilerM.run_lift,
    CheckedCompilerM.value_lift, CheckedCompilerM.run_pure,
    CheckedCompilerM.value_pure, CompilerM.run, CompilerM.value,
    freshRegM, freshReg, emitM]
  exact ⟨_, rfl⟩
...
theorem compileStmt_copy_projdst_zero_projsrc_offset_run
    ...
    (h_dval : CheckedCompilerM.value (placeToRegChecked RefKind.Mut dbase)
      (emit
          { nextReg := (emit { (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs) with
                  nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg + 1 }
                [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg)
                  (borrowRhs RefKind.Shared (blockSize τ) bOut.result.reg (pathOffset spath))]).nextReg + 1,
            nextLabel := (emit { (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs) with
                  nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg + 1 }
                [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg)
                  (borrowRhs RefKind.Shared (blockSize τ) bOut.result.reg (pathOffset spath))]).nextLabel,
            code := (emit { (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs) with
                  nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg + 1 }
                [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg)
                  (borrowRhs RefKind.Shared (blockSize τ) bOut.result.reg (pathOffset spath))]).code,
            placeRegMap := (emit { (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs) with
                  nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg + 1 }
                [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) cs).nextReg)
                  (borrowRhs RefKind.Shared (blockSize τ) bOut.result.reg (pathOffset spath))]).placeRegMap }
  ...
  simp only [compileStmtChecked, compileRExprPreChecked, h_proj_eqS, h_proj_eqD,
    CheckedCompilerM.run_bind, CheckedCompilerM.value_bind,
    CheckedCompilerM.run_lift, CheckedCompilerM.value_lift,
    CheckedCompilerM.run_pure, CheckedCompilerM.value_pure,
    h_root, h_bval, dif_neg h_so]
  simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
    cleanupInstrs, h_bclean, List.nil_append, List.cons_append,
    List.append_nil, List.reverse_cons, List.reverse_nil, List.map_cons,
    List.map_nil]
  simp only [h_dval, h_do, dif_pos,
    CheckedCompilerM.run_bind, CheckedCompilerM.value_bind,
    CheckedCompilerM.run_lift, CheckedCompilerM.value_lift,
    CheckedCompilerM.run_pure, CheckedCompilerM.value_pure]
  simp only [CompilerM.run, CompilerM.value, emitM, cleanupInstrs, h_dclean,
...
theorem compileStmt_copy_projdst_offset_projsrc_offset_run
  ...
  simp only [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
    ...
    List.append_nil, List.reverse_cons, List.reverse_nil, List.map_cons,
    List.map_nil]
  simp only [h_dval, dif_neg h_do,
    CheckedCompilerM.run_bind, CheckedCompilerM.value_bind,
    CheckedCompilerM.run_lift, CheckedCompilerM.value_lift,
    CheckedCompilerM.run_pure, CheckedCompilerM.value_pure]
  simp [CompilerM.run, CompilerM.value, freshRegM, freshReg, emitM,
    cleanupInstrs, h_dclean, emit_nil, borrowRhs]
...
theorem copy_projdst_zero_projsrc_offset_simulation
  ...
  · rw [if_neg h_fit] at h_step
    ...
    have h_instCS2 : ∀ q' instr,
        q' < 
                (emit { nextReg := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg + 1, nextLabel := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextLabel, code := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).code, placeRegMap := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).placeRegMap } [Instr.Assgn (Register.R (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)), Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).nextLabel →
        
                (emit { nextReg := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg + 1, nextLabel := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextLabel, code := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).code, placeRegMap := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).placeRegMap } [Instr.Assgn (Register.R (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)), Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).code q' = some instr →
        compProg q' = some instr := by
      intro q' instr h_lt h_code
      refine compileStmt_emitted_in_compProg h_comp h_csAt h_stmt h_stmtOut ?_ ?_
      · exact Nat.lt_of_lt_of_le h_lt h_incrCS2.nextLabel_le
  ...
  · rw [if_neg h_fit] at h_step
    ...
    · rename_i h_nb
      ...
      · rename_i perms₃ h_useMut_src
        ...
        have h_regbelow : RegisterBelow (emit { nextReg := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg + 1, nextLabel := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextLabel, code := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).code, placeRegMap := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).placeRegMap } [Instr.Assgn (Register.R (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)), Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]).nextReg (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1)) := by
          show (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1 < _
          csnorm
          omega
        have h_vreg : oseair.RegMap.lookup s_mid2.reg (Register.R ((CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1))
        ...
        have h_runC := (oseair_runN_add (n1 + 1 + 1) 1 s_osea compProg _ h_runB).trans h_run3
        have h_runD := (oseair_runN_add (n1 + 1 + 1 + 1) n2 s_osea compProg _ h_runC).trans h_drun
        have h_runE := (oseair_runN_add (n1 + 1 + 1 + 1 + n2) 1 s_osea compProg
          s_mid2 h_runD).trans h_run4
        have h_runF := (oseair_runN_add (n1 + 1 + 1 + 1 + n2 + 1) 1 s_osea compProg
          _ h_runE).trans h_run5
        have h_run := (oseair_runN_add (n1 + 1 + 1 + 1 + n2 + 1 + 1) 1 s_osea
          compProg _ h_runF).trans h_run6
        ...
        have h_sms' : SourceMemSim ρa ρt
          ...
          · have h_oe : rd.addr + PathTo.offset dpath = rd.addr + pathOffset dpath := by
              grind
            rw [h_oe]
            exact h_dom
          · rw [h_memchain]
        ...
        · have h_lbsT : LocalBindingSim ρa ρt s_mir.env
              { s_mid2 with
                  ...
                  mem := oseair.writeWordSeq s_mid2.mem
                    (rd.addr + pathOffset dpath) (oseair.readWordSeq s_mid1.mem (rs.addr + pathOffset spath) (blockSize τ)),
                  pc := s_mid2.pc + 1 + 1 + 1 } (emit { nextReg := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg + 1, nextLabel := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextLabel, code := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).code, placeRegMap := (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).placeRegMap } [Instr.Assgn (Register.R (emit { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg + 1, nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextLabel, code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).code, placeRegMap := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).placeRegMap } [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (borrowRhs RefKind.Shared (blockSize τ) sOut.result.reg (pathOffset spath))]).nextReg) (Rhs.Load (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg)), Instr.Die (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared B) csPrefix).nextReg) (blockSize τ)]) :=
...
theorem copy_projdst_zero_chainsrc_simulation
  ...
  · rw [if_neg h_fit] at h_step
    ...
    have h_incrDrun : StateIncr (CheckedCompilerM.run (placeToRegChecked RefKind.Mut (dbase))
      ...
      · exact StateIncr.refl _
    have h_instDst : ∀ q' instr,
        q' < (CheckedCompilerM.run (placeToRegChecked RefKind.Mut (dbase))
        (emit
          { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).nextReg + 1,
            nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).nextLabel,
            code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).code,
  ...
  · rw [if_neg h_fit] at h_step
    ...
    · rename_i h_nb
      ...
      · rename_i perms₃ h_useMut_src
            ...
            (emit
              ...
              [Instr.Assgn (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).nextReg)
                (Rhs.Load (layoutToTyVal τ) sOut.result.reg)])).nextReg)
                  (borrowRhs RefKind.Mut (blockSize τ) dOut.result.reg
                    (pathOffset path))])
              [Instr.RStore (layoutToTyVal τ) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).nextReg) (Register.R (CheckedCompilerM.run (placeToRegChecked RefKind.Mut (dbase))
            (emit
              { nextReg := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).nextReg + 1,
                nextLabel := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).nextLabel,
                code := (CheckedCompilerM.run (placeToRegChecked RefKind.Shared src) csPrefix).code,
        ...
        have h_runA := (oseair_runN_add n1 1 s_osea compProg s_mid1 h_srun).trans h_run1
        have h_runB := (oseair_runN_add (n1 + 1) n2 s_osea compProg _ h_runA).trans h_drun
        have h_runC := (oseair_runN_add (n1 + 1 + n2) 1 s_osea compProg s_mid2 h_runB).trans
          h_run2
        have h_runD := (oseair_runN_add (n1 + 1 + n2 + 1) 1 s_osea compProg _ h_runC).trans
          h_run3
        have h_run := (oseair_runN_add (n1 + 1 + n2 + 1 + 1) 1 s_osea compProg _ h_runD).trans
          h_run4
...
/-- RESIDUAL (sorried), NARROWED 2026-08-30 (temp-assignment
    ...
    For a PROJECTED destination the recursive dispatcher
    `copy_projdst_simulation` peels nested projections with the
    associativity transfers and closes a DEREF base at ZERO offset
    (`(*p).0 := copy y`, d62) with
    `copy_projdst_zero_chainsrc_simulation` — the same two-mother
    skeleton, with the projection contributing only a `+ 0` on the
    resolved address and a `pure` on the compiled side.

    Both offsets are covered: at ZERO the projection passes the chain's
    register through (d62), at NONZERO it wraps the `RStore` in its own
    `Borrow(Mut)`/`Die` — the BRIDGE 1 endgame (d63).

    A deref destination whose flattened SOURCE is proj-topped is closed
    at ZERO offset by `copy_chaindst_projsrc_zero_simulation` (d64) —
    the source projection passes the chain's register and cleanup
    through, so the READ simply happens one layer deeper.

    A deref destination whose flattened SOURCE is proj-topped at
    NONZERO offset (`*p := copy s.f`) is closed by
    `copy_chaindst_projsrc_offset_simulation` (d65) — the projection's
    `Borrow(Shared)`/`Die` bracket the READ (BRIDGE 1S), contiguously,
    because both retire before the destination lowering starts. The
    deref-destination arm is therefore TOTAL.

    PROJECTED destinations over a LOCAL base are closed too: a BOUND
    root falls to the same two leaves as a deref base, once they are
    generalized from `.deref P` to any canonical chain base — a bound
    local IS one (`PtrChain.base`), so this cost a hypothesis
    (`h_bound`, that `preparePlaceAssign` is a no-op) rather than a
    proof (d66/d67). An UNBOUND root is regime B-proj:
    `copy_projlocal_fresh_zero_simulation` and
    `copy_projlocal_fresh_offset_simulation` allocate the σ-sized root,
    run the source mother lemma at the POST-allocation states under
...
theorem copy_place_residual
    ...
    (h_run0 : ∀ cs, CheckedCompilerM.run (compileStmtChecked stmt0) cs
      = CheckedCompilerM.run
          (compileStmtChecked (Stmt.assign dst (.copy src))) cs)
    (h_val0 : ∀ cs so, CheckedCompilerM.value
        (compileStmtChecked (Stmt.assign dst (.copy src))) cs
        = Except.ok so →
      ∃ so', CheckedCompilerM.value (compileStmtChecked stmt0) cs
