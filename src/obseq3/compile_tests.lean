...
def rs_known_divergence_projsrc_mut : IO Unit := do
  let prog : Prog ΓRS :=
    [.assign (.proj tRS (.field ⟨0, by decide⟩ .nil)) (.constInit 1),
     ...
     .assign pRS (.ref (.Raw true) false [] tRS),
     -- park that very pointer in t.1, writing THROUGH it so its own item
     -- is not popped; the cast only changes the pointee type
     .assign (.proj (.deref pRS) (.field ⟨1, by decide⟩ .nil)) (.ptrCast pRS),
     -- the retag's range is the whole tuple, which CONTAINS the cell the
